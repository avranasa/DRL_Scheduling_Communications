...
class BaselineIntLPoracleMethod():
    ...
    def __init__(self, BW, EperSymb, env):
        self.BW = BW
    ...
    def __call__(self, env):  
        ...
        for sample in range(env.N_parallel_env):   
            ...
            for t in range(self.Tgreedy):
                self.ILPsolver += sum(Wth[sample][k][t]*self.x[k][t] for k in range(self.K))<=self.BW
...
def BaselineTest_intLPoracle(env, Resources):
    ...
    Scheduler = BaselineIntLPoracleMethod(BW, EperSymb, env)
